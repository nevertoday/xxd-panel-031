...
Create fresh PNG artwork with `references/031-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/031-source.md` completely immediately before building every generation request.
...
```text
/xxd-panel-031 <source-or-directory> --mode top-bottom,design-only \
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/031-source.md,
...
```text
~/Desktop/xxd/xxd-panel-031/<fresh-task>/
...
- the result follows `references/031-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
- `references/xxd-panel-031-prompt.zh-CN.md`
- `references/xxd-panel-031-prompt.en.md`

These files document the same minimal adapter. They never replace `references/031-source.md`.
